...
/* delay_ms 直接使用 mid_delay.h 中的函数 */
...
{
    // GPIO pins configured by SysConfig, no STM32 RCC/GPIO init needed
    uint8_t res = 0;
	...
	{	 
		...
		res=run_self_test();		//�Լ�
		if(res){
            printf("mpu_dmp_init�Լ�ʧ��\n");
            return 8;
        }  
		res=mpu_set_dmp_state(1);	//ʹ��DMP
		if(res){
            printf("mpu_dmp_initʹ��DMPʧ��\n");
            return 9;
        }
	}else {
        printf("mpu_init��ʼ��ʧ��\n");
