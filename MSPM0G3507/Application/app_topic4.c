 ...
 *
 * 钢球控制完整移植自电脑端 pc_serial_cascade_balance.py：
 *   外环：位置 PID -> 目标球速度；
 *   内环：球速度 PID -> 水管倾斜脉冲。
 *
 * 已实测参数：位置 P=1.2, I=0.79, D=0.15；
 *                 速度 P=8, I=0, D=0.2。
 * 球进入 O 点 +/-0.35 cm 后清空位置积分，避免低速静止后还被慢慢推走。
 *
...
#define POSITION_KP                   1.2f
#define POSITION_KI                   0.79f
#define POSITION_KD                   0.15f
#define POSITION_DEADBAND_CM          0.35f
#define POSITION_INTEGRAL_LIMIT       3.0f
#define TARGET_SPEED_LIMIT_CM_S       7.0f
...
#define CONTROL_OUTPUT_LIMIT         80.0f
...
static void topic45_run(bool full_lap)
{
    ...
    while (1) {
        ...
        if (!finished && (now_ms - last_control_ms >= XUNJI_CONTROL_PERIOD_MS)) {
        ...
        if (!finished && (now_ms - last_vision_ms > VISION_TIMEOUT_MS)) {
            /* 视觉断流时不继续使用旧坐标行驶。 */
            finished = true;
            fishpath_stop();
            (void)mid_stopwatch_stop();
            ...
            drv_uart_send_string("CAM LOST: car stopped\r\n");
        }
